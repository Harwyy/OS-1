...
#include <chrono>
#include <vector>
#include <thread>  // подключаем для использования std::thread

using namespace std;
using namespace chrono;

void runWriteTask(const string& filename, size_t blockSize, size_t totalBlocks) {
...
int main(int argc, char* argv[]) {
  ...
  vector<thread> threads;  // теперь этот контейнер корректно работает

  for (int i = 0; i < count; i++) {
    threads.push_back(thread(runWriteTask, filename, blockSize, totalBlocks));  // создание потока
  }
